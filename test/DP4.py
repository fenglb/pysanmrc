...
if __name__ == "__main__": 
    ...
    exp_b_c13 = np.array( [ 41.0, 26.3, 20.9, 34.6, 56.1, 40.0, 34.5, 41.9, 58.5, 37.4, 32.5, 36.9, 65.1, 40.0, 22.0, 23.0, 43.4] )
    scaledA = ScaleNMR( calc_a_c13, exp_b_c13 )
    for value in scaledA:
        print value
    scaledErrors = map( lambda x, y: x - y, exp_b_c13, scaledA )
    ...
    print CalculateCDP4( scaledErrors, meanC, stdevC )

    scaledA = ScaleNMR( calc_b_c13, exp_b_c13 )
    print "="*20
    for value in scaledA:
        print value
    scaledErrors = map( lambda x, y: x - y, exp_b_c13, scaledA )
